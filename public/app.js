...
document.getElementById('resetProgress')?.addEventListener('click',()=>{if(!confirm('Deseja realmente apagar sua Empresa Virtual, respostas e progresso desta jornada?'))return;state={...defaultState,answered:{},metrics:{...defaultState.metrics},labs:{},achievements:{}};save();localStorage.removeItem('storyplay-month-history');localStorage.removeItem('storyplay-goals');localStorage.removeItem('storyplay-events-state');location.reload()});
...
function loadExtraModules(){const styles=['/labs.css','/progress.css','/business.css','/monthly.css','/events.css'];styles.forEach(src=>{if(!document.querySelector(`link[href="${src}"]`)){const link=document.createElement('link');link.rel='stylesheet';link.href=src;document.head.appendChild(link)}});const scripts=['/labs.js','/progress.js','/business.js','/monthly.js','/events.js'];scripts.forEach(src=>{if(!document.querySelector(`script[src="${src}"]`)){const script=document.createElement('script');script.src=src;script.defer=true;document.body.appendChild(script)}})}
