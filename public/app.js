...
const defaultState={xp:0,answered:{},company:null,metrics:{revenue:0,clients:0,reputation:50},labs:{}};
let state;
try{state={...defaultState,...JSON.parse(localStorage.getItem('storyplay-state')||'{}')}}catch(e){state={...defaultState}}
if(!state.answered)state.answered={};if(!state.metrics)state.metrics={...defaultState.metrics};if(!state.labs)state.labs={};
...
function levelFromXP(xp){if(xp>=200)return'Nível 3 · Gestor';if(xp>=120)return'Nível 2 · Empreendedor';if(xp>=50)return'Nível 1 · Explorador';return'Nível 1 · Aprendiz'}
function renderProgress(){
 const xp=Number(state.xp||0);const knowledge=Math.min(100,Math.round(xp/250*100));
 ...
 const next=missions===0?'Primeira missão: descubra um problema que valha a pena resolver.':missions<2?'Continue o Episódio 1 e valide sua ideia.':missions<4?'Avance para o Episódio 2 e aprenda a formalizar.':labsDone<2?'Experimente os Laboratórios Empresariais e teste suas decisões.':'Boa evolução! Continue pelos laboratórios e administre sua Empresa Virtual.';
...
document.querySelectorAll('.decision-options').forEach(group=>{
 ...
 group.querySelectorAll('button[data-score]').forEach(btn=>btn.addEventListener('click',()=>{
  ...
  if(state.company){if(score===25){state.metrics.reputation=Math.min(100,Number(state.metrics.reputation||50)+3)}else if(score===0){state.metrics.reputation=Math.max(0,Number(state.metrics.reputation||50)-2)}}
...
function renderCompany(){
 ...
 if(!state.company){
  ...
  if(heroCompany)heroCompany.textContent='Ainda não criada';if(companyNameInput)companyNameInput.value='';if(companyCapitalInput){companyCapitalInput.value='';companyCapitalInput.placeholder='Ex.: 10000'}if(companySectorInput)companySectorInput.selectedIndex=0;
 ...
 const c=state.company;if(companyStatus){companyStatus.innerHTML='<strong>'+escapeHTML(c.name)+'</strong> está criada no segmento <strong>'+escapeHTML(c.sector)+'</strong>, com capital inicial de <strong>'+brl(c.capital)+'</strong>.';companyStatus.classList.add('success')}if(heroCompany)heroCompany.textContent=c.name;if(companyNameInput)companyNameInput.value=c.name;if(companySectorInput)companySectorInput.value=c.sector;if(companyCapitalInput)companyCapitalInput.value=c.capital;
 const counter=document.getElementById('companyNameCounter');if(counter)counter.textContent=c.name.length+' / '+companyNameInput.maxLength+' caracteres';document.getElementById('dashCapital').textContent=brl(c.capital);document.getElementById('dashRevenue').textContent=brl(state.metrics.revenue);document.getElementById('dashClients').textContent=state.metrics.clients||0;document.getElementById('dashReputation').textContent=(state.metrics.reputation||50)+'/100';
...
document.getElementById('saveCompany')?.addEventListener('click',()=>{const name=companyNameInput.value.trim();const sector=companySectorInput.value;const capital=Number(companyCapitalInput.value||0);if(name.length<2){companyStatus.textContent='Escolha um nome com pelo menos 2 caracteres.';companyStatus.classList.remove('success');companyNameInput.focus();return}if(!Number.isFinite(capital)||capital<1000){companyStatus.textContent='Informe um capital inicial de pelo menos R$ 1.000.';companyStatus.classList.remove('success');companyCapitalInput.focus();return}const isNew=!state.company;state.company={name,sector,capital};if(isNew){state.xp=Number(state.xp||0)+15;state.metrics.reputation=Math.max(50,Number(state.metrics.reputation||50))}save();renderProgress();renderCompany();companyStatus.scrollIntoView({behavior:'smooth',block:'center'})});

document.getElementById('resetProgress')?.addEventListener('click',()=>{if(!confirm('Deseja realmente apagar sua Empresa Virtual, respostas e progresso desta jornada?'))return;state={...defaultState,answered:{},metrics:{...defaultState.metrics},labs:{}};save();location.reload()});

window.storyplayAPI={
 awardXP(key,points){if(!key||state.labs[key])return false;state.labs[key]=true;state.xp=Number(state.xp||0)+Number(points||0);save();renderProgress();renderCompany();return true},
 getState(){return JSON.parse(JSON.stringify(state))},
 updateMetrics(partial={}){state.metrics={...state.metrics,...partial};save();renderCompany()}
};

function loadExtraModules(){if(!document.querySelector('link[href="/labs.css"]')){const link=document.createElement('link');link.rel='stylesheet';link.href='/labs.css';document.head.appendChild(link)}if(!document.querySelector('script[src="/labs.js"]')){const script=document.createElement('script');script.src='/labs.js';script.defer=true;document.body.appendChild(script)}}
